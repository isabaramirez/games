window.webInvaders = function(){
  ...
  function startGame(){
    ...
    document.addEventListener("keydown", e=>{
      if(e.key==="ArrowLeft" || e.key==="a") naveX-=speed;
      if(e.key==="ArrowRight" || e.key==="d") naveX+=speed;
      if(e.key==="ArrowUp" || e.key==="w") naveY-=speed;
      if(e.key==="ArrowDown" || e.key==="s") naveY+=speed;
      if(e.key===" ") disparar();
      if(e.key==="x") superDisparo();
      if(e.key==="+" || e.key==="ç") cleanup();
      if(e.key==="}") resumeGame();
    });
    ...
    function resumeGame(){
      ...
      enemigos.forEach(e=>e.el.style.display="block");
    }
  }
};
